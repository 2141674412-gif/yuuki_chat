...
async def _cmd_help(event: MessageEvent):
    ...
    if content in ("2", "二", "工具"):
        page = 2
    elif content in ("3", "三", "舞萌", "mai"):
        page = 3
    elif content in ("4", "四", "管理") and _is_admin:
        page = 4
    elif content in ("5", "五", "保险箱", "保险"):
        page = 5 if _is_admin else 4

    _nav_admin = "  >> /帮助 4 管理命令\n  >> /帮助 5 保险箱用法"
    _nav_user = "  >> /帮助 4 保险箱用法"
    _nav = _nav_admin if _is_admin else _nav_user
    _total = "1~5" if _is_admin else "1~4"

    pages = {
        1: f"""[ 命令列表 1/{'5' if _is_admin else '4'} - 日常 & 趣味 ]
...
{_nav}""",
        2: f"""[ 命令列表 2/{'5' if _is_admin else '4'} - 工具 ]
...
{_nav}""",
        3: f"""[ 命令列表 3/{'5' if _is_admin else '4'} - 舞萌DX ]
...
{_nav}""",
  ...
  >> /帮助 4 管理命令""",
    ...
    if page in pages:
        ...
        await _send(event, f"...没有这一页。/帮助 {_total}")
