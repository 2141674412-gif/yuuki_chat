...
async def _cmd_help(event: MessageEvent):
    ...
    _is_admin = check_superuser(str(event.user_id))

    page = 1
    if content in ("2", "二", "工具"):
        page = 2
    elif content in ("3", "三", "管理") and _is_admin:
        page = 3

    _total = 3 if _is_admin else 2

    _nav1 = "  >> /帮助 2 工具 & 系统" if _total == 2 else "  >> /帮助 2 工具 & 系统\n  >> /帮助 3 管理命令"
    _nav2 = "  >> /帮助 1 日常 & 趣味" if _total == 2 else "  >> /帮助 1 日常 & 趣味\n  >> /帮助 3 管理命令"
  ...
  签到积分
    /签到(qd,打卡) /积分(jf) /排行(ph)

  趣味
    /抽签(cq) /运势(ys) /成语(cy) /笑话(xh) /谜语(my) /戳我(cw)
    /点歌(dg,music,搜歌) 歌名

  记账 (发支付截图自动记账)
    /记(记账,记录) 午饭 25 /账单(账目,明细) /统计(汇总) /清空记账(清除记账)

  生日
    /生日 /设置生日(setbirthday,setbirth) MM-DD 昵称 /删除生日(delbirthday,delbirth)

  舞萌DX
    /mai(舞萌) b50 /mai(舞萌) b40 /mai(舞萌) 歌曲 歌名 /牌子(pz)
    /绑定(bd) /绑定水鱼(bdsy) 用户名 /绑定token /解绑(jb) /查机台 关键词

{_nav1}{_note}""",
        2: f"""[ 命令列表 2/{_total} - 工具 & 系统 ]

  搜索 & 翻译
    /搜索(ss,搜一下,查一查) 关键词 /搜图(st,图片搜索,imgsearch) 关键词
    /翻译(fy) 内容 /汇率 100美元

  天气
    /天气(tq,weather) 城市 /绑定天气(bdtq,bindweather) 城市 /我的天气(wdtq,myweather,setcity)

  实用工具
    /计算器(jsq,calc,计算) 表达式
    /提醒(tx) 30分钟 xxx /提醒(tx) 明天8点 xxx
    /提醒列表(txlb,提醒历史,remindlist) /取消提醒(qxtx)
    /定时 08:00 早安 /词云(cy2)

  保险箱 (私聊)
    /设置密码 /存 名称|密码 内容 /取 名称|密码
    /密码列表 密码

{_nav2}{_note}""",
        3: f"""[ 命令列表 3/3 - 管理命令 ] (仅管理员)

  群管
    /禁言 @某人 [时长] /踢 @某人 /撤回
    /加过滤(addfilter,加屏蔽) /删过滤(delfilter,删屏蔽) /过滤模式(filtermode)
    /设置欢迎(setwelcome)

  用户管理
    /拉黑(加黑,加黑名单) /解黑(移黑,解除拉黑) /加白(加白名单) /移白(删白名单)
    /黑名单 /白名单

  系统
    /更新(更新状态,更新日志) /重启 /run(执行,exec) 命令
    /诊断(自检,diag,状态) /自测(zc) /测试命令(test)
...
{_note}""",
    ...
    if page in pages:
        ...
        await _send(event, f"...没有这一页。/帮助 1~{_total}")

help_cmd = _register("帮助", _cmd_help, aliases=["help", "bz"])
...
admin_help_cmd = _register("管理帮助", _cmd_admin_help, aliases=["adminhelp", "glbz"])
...
sticker_list_cmd = _register("表情包", _cmd_sticker_list, aliases=["stickers", "bqb"])
...
selftest_cmd = _register("自测", _cmd_selftest, aliases=["zc"], admin_only=True)
...
test_cmd = _register("测试命令", _cmd_test_commands, aliases=["test"], admin_only=True)
...
persona_cmd = _register("人设", _cmd_persona, aliases=["rs"])
...
reset_cmd = _register("重置", _cmd_reset, aliases=["reset", "cz"])
...
poke_cmd = _register("戳我", _cmd_poke, aliases=["cw"])
...
joke_cmd = _register("笑话", _cmd_joke, aliases=["xh"])
...
riddle_cmd = _register("谜语", _cmd_riddle, aliases=["my"])
...
draw_cmd = _register("抽签", _cmd_draw, aliases=["cq"])
...
fortune_cmd = _register("运势", _cmd_fortune, aliases=["ys"])
...
idiom_cmd = _register("成语", _cmd_idiom, aliases=["cy"])
...
music_cmd = _register("点歌", _cmd_music, aliases=["music", "搜歌", "dg"])
...
affinity_cmd = _register("好感度", _cmd_affinity, aliases=["affinity", "hgd"])
