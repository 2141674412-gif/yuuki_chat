...
async def _cmd_help(event: MessageEvent):
  ...
  舞萌DX
    /mai b50 /mai b40 /mai 歌曲 歌名 /牌子
    /绑定(绑定水鱼,绑定token) 好友码 /解绑 /查机台 关键词

{_nav1}{_note}""",
        2: f"""[ 命令列表 2/{_total} - 工具 & 系统 ]

  搜索 & 翻译
    /搜索(搜一下,查一查) 关键词 /搜图(图片搜索,imgsearch) 关键词
    /翻译 内容 /汇率 100美元

  天气
    /天气(weather) 城市 /绑定天气(bindweather) 城市 /我的天气(myweather,setcity)

  实用工具
    /计算器 表达式
    /提醒 30分钟 xxx /提醒 明天8点 xxx
    /提醒列表(提醒历史,remindlist) /取消提醒
    /定时 08:00 早安 /词云
