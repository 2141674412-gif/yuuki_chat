...
async def _cmd_restart(event: MessageEvent):
    ...
    restart_data = {'user_id': user_id, 'reason': '手动重启', 'timestamp': str(int(__import__('time').time()))}
    ...
    try:
        ...
        if sys.platform == 'win32':
            ...
            subprocess.Popen([sys.executable, bot_path], cwd=project_dir, start_new_session=True)
        sys.stdout.flush()
        sys.stderr.flush()
        os._exit(0)
    except OSError as e:
