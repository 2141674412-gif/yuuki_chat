...
def _get_pil_modules():
    """延迟加载 PIL 模块"""
    global _Image, _ImageDraw, _ImageFont
    if _Image is None:
...
def _get_cover_cache_dir():
    ...
    if _COVER_CACHE_DIR is None:
        _COVER_CACHE_DIR = os.path.join(_get_plugin_dir(), "cover_cache")
...
# ── 成就标签延迟加载 ──────────────────────────────────
_sorted_achiev_labels = None

def _get_sorted_achiev_labels():
    """延迟获取排序后的成就标签"""
    global _sorted_achiev_labels
    if _sorted_achiev_labels is None:
        try:
            from .config import ACHIEV_LABELS
            _sorted_achiev_labels = sorted(ACHIEV_LABELS.items(), key=lambda x: x[0], reverse=True)
        except ImportError:
            _sorted_achiev_labels = []
    return _sorted_achiev_labels
...
def get_achiev_label(achievements):
    for threshold, label in _get_sorted_achiev_labels():
...
def make_default_cover(size=(100, 100), title=""):
    """生成默认封面（显示歌曲首字母）"""
    Image, ImageDraw = _get_pil_modules()[:2]
    ...
    draw.text((int((w - tw) / 2), int((h - 32) / 2)), initial, fill=(100, 100, 140, 200), font=font)
    return img


async def download_cover(http_client, song_id, size=(100, 100)):
    """异步下载歌曲封面，返回 PIL Image 或 None"""
    if not song_id:
        return None

    Image = _get_pil_modules()[0]
    ...
    if LOCAL_COVER_DIR and os.path.isdir(LOCAL_COVER_DIR):
        ...
        display_id = sid - 10000 if 10001 <= sid <= 19999 else sid
    ...
    # ---- 磁盘缓存 ----
    cache_path = os.path.join(_get_cover_cache_dir(), f"{song_id}.png")
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                delay = (2 ** attempt)
                logger.warning(f"[封面下载失败] song_id={song_id}, 第{attempt+1}次重试, 等待{delay}s, error={e}")
